...
/*int32_t kn_proactor_unbind(kn_fd_t s){
	if(s->type == STREAM_SOCKET || s->type == DGRAM_SOCKET){
		if(!s->proactor) return -1;
		if(0 != s->proactor->UnRegister(s->proactor,s))
			return -1;
		return 0;
	}else
		return -1;
...
kn_sockaddr*  kn_fd_local_addr(kn_fd_t s){
	assert(s);	
	if(s->type == STREAM_SOCKET || s->type == DGRAM_SOCKET){
...
kn_sockaddr*  kn_fd_remote_addr(kn_fd_t s){
	assert(s);	
	if(s->type == STREAM_SOCKET || s->type == DGRAM_SOCKET){
		return &((kn_datasocket*)s)->addr_remote;
	}
	return NULL;
}
