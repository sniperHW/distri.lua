...
extern msgdisp_t super_msgdisp;//superservice的msgdisp_t
...
void super_destroyer(super_player_t splayer)
{
	msgdisp_t disp = (msgdisp_t)tls_get(MSGDISCP_TLS);
	if(disp == super_msgdisp)
	{
		//在superservice中，直接执行销毁过程
		splayer->super_fn_destroy((void*splayer));
	}else
	{
		...
		msg->fn_function = splayer->super_fn_destroy;
